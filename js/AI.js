...
ai.ABSearch=function(x,y,depth,alpha,beta){
    ...
    x=tmpqueue[i];
    y=tmpqueue[--i];
    var score=-this.ABSearch(x,y,depth,-b,-alpha);
    ...
    while(i--){
        x=tmpqueue[i];
        y=tmpqueue[--i];
        score=-this.ABSearch(x,y,depth,-b,-alpha);
        this.desimulate(x,y,depth%2);
        if(alpha<score && score<beta){
            score=-this.ABSearch(x,y,depth,-beta,-alpha);
            this.desimulate(x,y,depth%2);
        }
        if(score>alpha){
            alpha=score;
